...
from database import create_tables, get_user_name, save_user_name, save_user_plan, get_user_plan

# Замените 'YOUR_BOT_TOKEN' на токен, полученный от @BotFather
TOKEN = '7947948717:AAF_yeOpDDLoOCrTUWQRzb_akKx05xWpNVU'
plans = {'Default': 'План - Default:\n Плана date  d.w\n   Подъем:\n1) ......\n2) ......\n3) ......\n4) ......\n5) ......\n   Сон:',
'Hard': 'План - Hard:\n Плана date  d.w\n   Подъем:\n1) ...... () Время\n2) ......() Время\n3) ......() Время\n4) ......() Время\n5) ......() Время\n   Сон:' }
...
async def view_plans_command(message: types.Message):
    user_id = message.from_user.id
    user_plans = get_user_plan(user_id)  # Получаем план пользователя
    print(user_plans)
    if user_plans:
        for plan in user_plans:
            await message.reply(f"Ваши план:\n{plan}")
    else:
...
async def handle_plan_selection(message: types.Message):
    global chech_name, chech_plan, chech_create_plan
    user_id = message.from_user.id
    current_plan = get_user_plan(user_id)
    if chech_name:
        user_id = message.from_user.id
        ...
        save_user_plan(user_id, message.text)  # Предполагается, что у вас есть функция для сохранения плана
