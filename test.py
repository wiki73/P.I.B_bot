...
@dp.message(state=Form.waiting_for_nick)
async def process_nick(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    save_user_name(user_id, message.text)  # Сохраняем ник пользователя
    await message.reply(f"Спасибо, {message.text}! Теперь я буду называть тебя так.")
